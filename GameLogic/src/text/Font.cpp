...
Font::Font(File file, float size, bool preload) : file(std::move(file)), size(size), preloaded(preload) {
	...
	this->font = ftgl::texture_font_new_from_file(this->atlas, this->size, this->file.getAbsolutePath().string().c_str());
...
Font::Font(std::string file, float size, bool preload) : Font(std::move(File(file)), size, preload) {
...
float Font::getHeight(const std::string& text) const {
	...
	for (int i = 0; i < text.size(); i++) {
		...
		texture_glyph_t* glyph = texture_font_get_glyph(this->font, &text[i]);
		float height = glyph->height / 1.0f;
		float offset = glyph->offset_y / 1.0f - height;
