...
void Client::start() {
	...
	this->client_thread = std::thread([&]() {
		...
		while (this->running) {
			...
			this->event_status = enet_host_service(this->client, &this->event, Main::networking_event_max_delay);
						...
						*/
						...
						switch (type) {
							...
							case PacketType::Level:
								...
								Main::level_content = (char*) data;
								Main::loaded_level = true;
...

							{
								...
								Main::player->level->addTile((new Bomb(Main::player->level->gridToWorldPos(index), TileProperties::bomb_time, nullptr))->setIndex(index));
							...
							case PacketType::Player:
								...
								if (Main::enemy_player != nullptr) {
									...
									memcpy(&Main::enemy_player->ghost_player.position, data, 12);
									memcpy(&Main::enemy_player->ghost_player.velocity, data + 12, 12);
									memcpy(&Main::enemy_player->hp, data + 12 + 12, 4);
									memcpy(&Main::enemy_player->rotation, data + 12 + 12 + 4, 1);
									memcpy(&Main::enemy_player->state, data + 12 + 12 + 4 + 1, 1);
									memcpy(&Main::enemy_player->offset, data + 12 + 12 + 4 + 1 + 1, 8);
									memcpy(&Main::enemy_player->colorMix, data + 12 + 12 + 4 + 1 + 1 + 8, 12);
									memcpy(&Main::enemy_player->mixColor, data + 12 + 12 + 4 + 1 + 1 + 8 + 12, 1);
...
void Client::addPackets() {
	...
	if (Main::player != nullptr) {
		...
		this->addPacket(new PlayerPacket(Main::player));
